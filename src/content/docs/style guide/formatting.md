---
title: Formatting
description: How to format crochet patterns with title case titles, materials lists, gauge, difficulty levels, stitch abbreviations, and copyright notices using a standard section structure
---

Pattern formatting means presenting your instructions in a clear, organized way. The goal is to make it easy for crocheters to follow and create the same finished item.


## What are the key sections of a well-formatted crochet pattern?
Every crochet pattern should include these key elements in order.

### Intro
A brief description of the pattern and what the finished object will be.

### Pattern Title
Use Title Case (e.g., "Sunrise Mosaic Shawl"). Keep it descriptive and memorable.

### Difficulty Level
Indicate the skill level required.

### Materials List
A detailed list including yarn specifications (type, weight, color, yardage), hook size, and notions.

### Gauge
Stitch and row count within a specific measurement. Crucial for achieving correct size.

### Pattern Sizes
For multiple sizes, outline the dimensions for each variation clearly.

### Stitches and Abbreviations
List all stitches used with their abbreviations. Follow Craft Yarn Council standards.

### Notes
Important information or tips for working the pattern.

### Written Pattern
Step-by-step instructions organized by row or round.

### Photos and Visuals
Images of the finished project and progress photos or diagrams.

### Copyright Information
State how the pattern can and cannot be used.

**Example:**
See the [sweater pattern template](/templates/sweater-pattern-template/), [sock pattern template](/templates/sock-pattern-template/), and [other templates](/templates/) for reusable starting points.